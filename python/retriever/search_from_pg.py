...
def run_search_queries(conn, query_ids, query_embeddings, query_embeddings_binary, logger, k=10, num_queries=100):
    logger.info(f"Running {num_queries} search queries (retrieving top {k} results each)")
    ...
    for i, (query_id, query_embedding) in enumerate(tqdm(zip(subset_query_ids, subset_query_embeddings))):
        ...
        results["fp32"]["results"].append({
            "query_id": query_id,
            "retrieved": [{"id": row[0], "text": row[1], "distance": row[2]} for row in query_results]
        })
    ...
    for i, (query_id, query_embedding) in enumerate(tqdm(zip(subset_query_ids, subset_query_embeddings_binary))):
...
def calculate_result_agreement(search_results, logger):
    ...
    for i in range(num_queries):
        fp32_ids = [item["id"] for item in search_results["fp32"]["results"][i]["retrieved"]]
        halfvec_ids = [item["id"] for item in search_results["halfvec"]["results"][i]["retrieved"]]
        bit_ids = [item["id"] for item in search_results["bit"]["results"][i]["retrieved"]]
        
        fp32_halfvec_jaccard = len(set(fp32_ids).intersection(set(halfvec_ids))) / len(set(fp32_ids).union(set(halfvec_ids)))
        fp32_bit_jaccard = len(set(fp32_ids).intersection(set(bit_ids))) / len(set(fp32_ids).union(set(bit_ids)))
        halfvec_bit_jaccard = len(set(halfvec_ids).intersection(set(bit_ids))) / len(set(halfvec_ids).union(set(bit_ids)))
...
def evaluate_ranking_correlation(search_results, logger):
    ...
    for i in range(num_queries):
        ...
        if len(common_fp32_halfvec) > 1:
            fp32_halfvec_ranks = ([fp32_ranks[doc] for doc in common_fp32_halfvec], 
        ...
        if len(common_fp32_bit) > 1:
            fp32_bit_ranks = ([fp32_ranks[doc] for doc in common_fp32_bit], 
        ...
        if len(common_halfvec_bit) > 1:
            halfvec_bit_ranks = ([halfvec_ranks[doc] for doc in common_halfvec_bit], 
...
def save_metrics_to_markdown(performance_metrics, agreement_metrics, correlation_metrics, logger):
    ...
    with open(filename, "w") as f:
        ...
        for label, accessor in perf_metrics_to_display:
            ...
            if label.endswith("Per Second") or label.endswith("Queries"):
                f.write(f"| {label} | {fp32_val:.2f} | {halfvec_val:.2f} | {bit_val:.2f} |\n")
            else:
                f.write(f"| {label} | {fp32_val:.3f} | {halfvec_val:.3f} | {bit_val:.3f} |\n")
        
        f.write("\n## Result Agreement (Jaccard Similarity)\n\n")
        f.write("| Comparison | Jaccard Similarity |\n")
        f.write("|------------|--------------------|\n")
        f.write(f"| FP32 vs HalfVec (fp16) | {agreement_metrics['fp32_vs_halfvec']:.4f} |\n")
        f.write(f"| FP32 vs Bit | {agreement_metrics['fp32_vs_bit']:.4f} |\n")
        f.write(f"| HalfVec (fp16) vs Bit | {agreement_metrics['halfvec_vs_bit']:.4f} |\n")
        
        f.write("\n## Ranking Correlation (Spearman's Rho)\n\n")
        f.write("| Comparison | Correlation |\n")
        f.write("|------------|-------------|\n")
        f.write(f"| FP32 vs HalfVec (fp16) | {correlation_metrics['fp32_vs_halfvec']:.4f} |\n")
        f.write(f"| FP32 vs Bit | {correlation_metrics['fp32_vs_bit']:.4f} |\n")
        f.write(f"| HalfVec (fp16) vs Bit | {correlation_metrics['halfvec_vs_bit']:.4f} |\n")
        ...
        f.write("### Performance\n\n")
        f.write(f"- HalfVec (fp16) provides a **{halfvec_speedup:.1f}%** speedup compared to FP32 vectors\n")
        f.write(f"- Bit vectors provide a **{bit_speedup:.1f}%** speedup compared to FP32 vectors\n\n")
        ...
        f.write(f"- HalfVec (fp16) vectors return **{fp32_halfvec_agreement:.1f}%** of the same results as FP32 vectors\n")
        f.write(f"- Bit vectors return **{fp32_bit_agreement:.1f}%** of the same results as FP32 vectors\n\n")
        
        f.write("### Recommendations\n\n")
        
        if halfvec_speedup > 20 and fp32_halfvec_agreement > 90:
            f.write("- **HalfVec (fp16) is recommended** for this workload: significant performance improvement with minimal impact on result quality\n")
        elif halfvec_speedup > 0:
            f.write("- HalfVec (fp16) provides some performance benefits with reasonable result quality\n")
        
        if bit_speedup > 50 and fp32_bit_agreement > 70:
            f.write("- **Bit vectors are recommended** for high-throughput scenarios where approximate results are acceptable\n")
        elif bit_speedup > 0 and fp32_bit_agreement < 50:
            f.write("- Bit vectors provide speed but with significant changes in results - consider using with reranking\n")
...
def main():
    ...
    try:
        ...
        dimensions = 384  
        query_embeddings, query_embeddings_binary = encode_queries(query_texts, dimensions, logger)
        ...
        for emb_type in performance_metrics:
            ...
            logger.info(f"\n{type_label}:")
            logger.info(f"  Avg Query Time: {performance_metrics[emb_type]['avg_query_time']*1000:.3f} ms")
            logger.info(f"  p99 Query Time: {performance_metrics[emb_type]['p99_query_time']*1000:.3f} ms")
            logger.info(f"  Queries Per Second: {performance_metrics[emb_type]['queries_per_second']:.2f}")
        
        logger.info("\n=== RESULT QUALITY SUMMARY ===")
        logger.info(f"  FP32 vs HalfVec (fp16) agreement: {agreement_metrics['fp32_vs_halfvec']:.4f}")

...
def save_metrics_to_markdown(results, logger, include_halfvec=True, include_bit=True):
    ...
    with open(filename, "w") as f:
        ...
        for emb_type in emb_types:
            ...
            f.write(
                f"- **Insert Time:** {results[emb_type]['insert_time']:.2f} seconds\n"
            )
            f.write(
                f"- **Index Creation Time:** {results[emb_type]['index_time']:.2f} seconds\n"
            )
            f.write(
                f"- **Table Size:** {results[emb_type]['table_size']/1024/1024:.2f} MB\n"
            )
            f.write(
                f"- **Index Size:** {results[emb_type]['index_size']/1024/1024:.2f} MB\n"
            )
...
def process_embeddings(cursor, logger, dimensions=384):
    ...
    halfvec_results = process_halfvec_vectors(
        cursor, logger, df, vector_columns, dimensions
    )
...
def process_fp32_vectors(cursor, logger, df, vector_columns, dimensions):
    ...
    for index, row in df.iterrows():
        ...
        text = row["text"]
        vector_str = (
            "[" + ",".join([str(float(row[col])) for col in vector_columns]) + "]"
        )

        cursor.execute(
            f"INSERT INTO {table_name} (id, text, embedding) VALUES (%s, %s, %s::vector)",
    ...
    logger.info(
        f"Inserted {total_vectors} fp32 embeddings in {insert_time:.2f} seconds"
    )
...
def process_halfvec_vectors(cursor, logger, df, vector_columns, dimensions):
    ...
    for index, row in df.iterrows():
        ...
        cursor.execute(
            f"INSERT INTO {table_name} (id, text, embedding) VALUES (%s, %s, %s::halfvec)",
            (id_val, text, vector_str),
        )
        if index % 100 == 0:
    ...
    insert_time = time.time() - start_insert_time
    logger.info(
        f"Inserted {total_vectors} halfvec embeddings in {insert_time:.2f} seconds"
    )
...
def process_bit_vectors(cursor, logger, df, vector_columns, dimensions):
    ...
    for index, row in df.iterrows():
        ...
        bit_string = "".join(
            ["1" if float(row[col]) > 0 else "0" for col in vector_columns]
        )

        cursor.execute(
            f"INSERT INTO {table_name} (id, text, embedding) VALUES (%s, %s, %s::BIT({dimensions}))",
